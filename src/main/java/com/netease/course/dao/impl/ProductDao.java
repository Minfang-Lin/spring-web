...
//	@Options(useGeneratedKeys=true, keyProperty="0", keyColumn = "id")
	...
	public boolean insertPurchase(int contentId, int personId, int price, long time);
	...
	@Select("SELECT id, contentId, personId, price, time FROM trx WHERE contentId=#{0}")
	public Trx getTrxByContentId(int contentId);
	
	@Select("SELECT id, contentId, personId, price, time FROM trx WHERE contentId=#{0} AND personId=#{1}")
